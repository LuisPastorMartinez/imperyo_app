...
def check_password():
    ...
    def authenticate_user():
        ...
        if st.session_state["username_input"] == correct_username and hashed_input_password == correct_password_hash:
            ...
            # La actualización de st.session_state ya provoca un rerun
        else:
...
if check_password():
    ...
    def limpiar_telefono(numero):
        """Convierte el número a string y limpia formatos, manteniendo 9 dígitos si son válidos."""
        ...
        # Tomar los primeros 9 dígitos si hay más
        if len(numero_limpio) > 9:
    ...
    def unify_dataframe_columns(df):
        ...
        # Priorizar la columna 'Telefono' si existe
    ...
    def get_ordered_dataframe(df_to_order, collection_name):
        default_order = {
            'pedidos': [
                ...
                'Tipo de pago', 'Adelanto', 'Observaciones', 'Inicio Trabajo', 'Trabajo Terminado', 'Cobrado', 'Retirado', 'Pendiente'
            ],
            'gastos': [
        ...
        with st.form(f"form_{'modificar' if is_modifying else 'guardar'}_pedido", clear_on_submit=not is_modifying):
            ...
            with col1:
                ...
                telefono = st.text_input("Teléfono (9 dígitos)", 
                                        value=str(telefono_val) if pd.notna(telefono_val) else "",
                                        key="input_telefono",
                                        max_chars=9,
            ...
            return submitted, {
                ...
                'Producto': producto,
                'Cliente': cliente,
                'Telefono': st.session_state.input_telefono,
                'Club': club,
                'Talla': talla,
                'Tela': tela,
    ...
    def handle_save_pedido():
        ...
        if submitted:
            telefono_ingresado = form_data['Telefono'].strip()
            ...
            if telefono_ingresado and not telefono_limpio:
                st.error("El teléfono debe contener exactamente 9 dígitos numéricos. Ejemplo: 622019738")
                return
