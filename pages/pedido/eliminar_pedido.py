...
if st.session_state.delete_step == 0:
    if st.button("Eliminar Pedido", key="btn_eliminar", help="Pulsa para iniciar eliminación"):
        st.session_state.delete_step = 1
    st.markdown("<script>var el = window.parent.document.querySelector('button[kind=secondary]'); if (el) el.classList.add('green');</script>", unsafe_allow_html=True)

# Paso 2 - Amarillo
elif st.session_state.delete_step == 1:
    if st.button(f"¿Seguro eliminar pedido N° {pedido['ID']}?", key="btn_confirmar", help="Confirmar eliminación"):
        st.session_state.delete_step = 2
    st.markdown("<script>var el = window.parent.document.querySelector('button[kind=secondary]'); if (el) el.classList.add('yellow');</script>", unsafe_allow_html=True)

# Paso 3 - Rojo y eliminar
elif st.session_state.delete_step == 2:
    if st.button(f"Pedido a eliminar N° {pedido['ID']}", key="btn_borrar", help="Borrar definitivamente"):
        try:
    ...
    st.markdown("<script>var el = window.parent.document.querySelector('button[kind=secondary]'); if (el) el.classList.add('red');</script>", unsafe_allow_html=True)
