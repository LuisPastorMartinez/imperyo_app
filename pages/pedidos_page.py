...
def show_pedidos_page(df_pedidos, df_listas):
            ...
            if st.form_submit_button("Guardar Nuevo Pedido"):
                if not cliente or not telefono or not producto or precio <= 0:
                    ...
                    try:
                        ...
                        # Validación de tipos para Streamlit DataFrame
                        for col in df_pedidos.columns:
                            if df_pedidos[col].apply(lambda x: isinstance(x, dict) or isinstance(x, list)).any():
                                st.error(f"La columna {col} tiene tipos incompatibles para mostrar en Streamlit.")
    ...
    with tab3:
        ...
        st.info("Funcionalidad de modificar pendiente de desarrollo.")

    # ===== Pestaña 4: Eliminar Pedido =====
    with tab4:
        st.subheader("Eliminar Pedido")
        st.info("Funcionalidad de eliminar pendiente de desarrollo.")