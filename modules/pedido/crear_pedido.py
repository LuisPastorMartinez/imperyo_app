...
from utils.firestore_utils import get_next_id, save_dataframe_firestore, save_single_document_firestore
...
def show_create(df_pedidos, df_listas):
    ...
    if 'ultima_pagina' not in st.session_state:
        ...
        if st.session_state.ultima_pagina != "Crear":
            ...
            keys_to_delete = [
                ...
                "pendiente_", "empezado_", "cobrado_", "tipo_registro"
            ]
    ...
    st.info(f"📅 **Año del registro:** {año_actual}")

    # --- SELECCIÓN DE TIPO DE REGISTRO ---
    tipo_registro = st.radio(
        "¿Qué tipo de registro deseas crear?",
        options=["Pedido formal", "Posible cliente"],
        index=0,
        horizontal=True,
        key="tipo_registro"
    )
    ...
    if tipo_registro == "Posible cliente":
        # === FORMULARIO PARA POSIBLE CLIENTE ===
        st.markdown("### 👤 Datos del posible cliente")
        with st.form("posible_cliente_form"):
            cliente = st.text_input("Nombre*", placeholder="Nombre o empresa")
            telefono = st.text_input("Teléfono*", max_chars=9, placeholder="Ej: 612345678")
            club = st.text_input("Club / Equipo", placeholder="Si aplica")
            descripcion = st.text_area("Mensaje / Consulta", placeholder="¿Qué necesita? ¿Qué preguntó?")
            observaciones = st.text_area("Notas internas", placeholder="Seguimiento, estado de la conversación, etc.")

            submitted = st.form_submit_button("✅ Guardar Posible Cliente", type="primary", use_container_width=True)
            ...
            if submitted:
                ...
                nuevo_registro = {
                    ...
                    'Club': convert_to_firestore_type(club) if club else None,
                    'Mensaje': convert_to_firestore_type(descripcion) if descripcion else None,
                    'Observaciones': convert_to_firestore_type(observaciones) if observaciones else None,
                    'Fecha contacto': convert_to_firestore_type(datetime.now().date()),
                    'Año': año_actual
                }

                with st.spinner("💾 Guardando posible cliente..."):
                    if save_single_document_firestore(nuevo_registro, 'posibles_clientes'):
                        st.success("🎉 ¡Posible cliente guardado correctamente!")
                        st.balloons()
                        time.sleep(1)
                        st.rerun()
                    else:
                        st.error("❌ Error al guardar. Por favor, inténtelo de nuevo.")
        ...
        with st.form("nuevo_pedido_form", clear_on_submit=False):
            ...
            # ✅ ESTADOS: Reordenado y sin ninguno activo por defecto (index=-1 no permitido, así que usamos un truco)
            st.write("### 🏷️ Estado del pedido")
            estado = st.radio(
                "Selecciona el estado inicial:",
                options=["Empezado", "Cobrado", "Pendiente"],  # ✅ ORDEN CORREGIDO
                index=0,  # Por defecto "Empezado", pero puedes cambiar a -1 si Streamlit lo permite (no lo permite, así que 0 es seguro)
                horizontal=True,
